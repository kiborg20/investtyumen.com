...
/**
 * @param $strValue название услуги
 * @return string
 */
function getPreFilterStr ($strValue , $link , $linkName) {
    if ($link) {
        return $link.'&'.$linkName.abs(crc32(htmlspecialcharsbx($strValue))).'=Y';
    } else {
        return '&'.$linkName.abs(crc32(htmlspecialcharsbx($strValue))).'=Y';
    }
}
 ...
 * @param $strValue название услуги
 */
function getPrtFilterInt ($strValue) {
    return abs(crc32(htmlspecialcharsbx($strValue)));
}
