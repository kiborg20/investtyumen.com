...
while ($arFields = $res->GetNext())
{
    ...
    $LINKS_VALUES = [];
    $resBtnLinksProp = CIBlockElement::GetProperty((int)$arParams["IBLOCK_ID"], $arFields["ID"], "sort", "asc", array("CODE" => "IT_LINKS"));
    while ($ob = $resBtnLinksProp->GetNext())
    {
        if ($ob['DESCRIPTION']) {
            $LINKS_VALUES[] = $ob;
        }
    ...
    if (!empty($LINKS_VALUES)) {
        $arFields["PROPERTY_IT_LINKS_VALUE"] = $LINKS_VALUES;
    }
